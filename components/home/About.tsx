...
export default function About() {
  ...
  return (
    <section id="about" className="bg-[#090909] py-28 md:py-36">
      <div className="mx-auto w-full max-w-6xl px-8">
        <div className="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-20">
          ...
          <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.7, delay: 0.1 }}>
            <p className="text-sm uppercase tracking-[0.55em] text-purple-400">{t.about.eyebrow}</p>
            ...
            <ul className="mt-10 flex flex-wrap gap-3 border-y border-white/10 py-7">
              {copy.services.map((service) => <li key={service} className="text-sm text-purple-300">{service}</li>)}
            </ul>

            <Link href="#reservation" className="group mt-10 inline-flex items-center gap-3 text-sm font-medium uppercase tracking-[0.24em] text-white transition hover:text-purple-300">
              {t.about.discover}
