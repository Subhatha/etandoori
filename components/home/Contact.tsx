...
export default function Contact() {
  ...
  const cardClass = "rounded-2xl border border-white/10 bg-[#15120f] p-6 sm:p-8";

  return (
    <section id="contact" className="scroll-mt-24 bg-[#0c0a09] py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-xs uppercase tracking-[0.35em] text-brand-400">{t.contact.eyebrow}</p>
          ...
          <ul className="mt-6 flex flex-wrap justify-center gap-3">
            {[...copy.services, copy.halal].map((service) => <li key={service} className="rounded-full border border-brand-500/25 px-4 py-2 text-sm text-brand-300">{service}</li>)}
          </ul>
        </div>
        <div className="mt-12 grid gap-5 lg:grid-cols-[1fr_1fr_1.4fr]">
          <div className={cardClass}>
            <Phone aria-hidden="true" className="text-brand-400" size={22} />
            <h3 className="mt-5 text-xl font-semibold">{copy.phone}</h3>
            <a href={restaurant.phoneHref} className="mt-4 inline-block text-xl text-brand-300 hover:text-white">{restaurant.phone}</a>
            <p className="mt-3 text-sm leading-6 text-zinc-400">{copy.call}</p>
          </div>
          <div className={cardClass}>
            <MapPin aria-hidden="true" className="text-brand-400" size={22} />
            ...
            <a href={restaurantDirectionsUrl} target="_blank" rel="noopener noreferrer" className="mt-4 inline-block text-sm text-brand-300 hover:text-white">{copy.directions} ↗</a>
          </div>
          <div className={cardClass}>
            <Clock3 aria-hidden="true" className="text-brand-400" size={22} />
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-[#15120f]">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 p-5 sm:px-8">
            ...
            <a href={restaurantDirectionsUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-brand-300 hover:text-white">{copy.directions} ↗</a>
          </div>
        </div>
        <details className="mt-6 rounded-2xl border border-white/10 bg-[#15120f] p-6 sm:p-8">
          ...
          <table className="mt-5 w-full text-left text-sm">
            ...
            <tbody>{deliveryZones.map((zone) => <tr key={zone.towns} className="border-b border-white/5 last:border-0"><th scope="row" className="py-3 pr-4 font-normal text-zinc-300">{zone.towns}</th><td className="whitespace-nowrap py-3 text-right text-brand-300">{formatMenuPrice(zone.minimum, language)}</td></tr>)}</tbody>
          </table>
        </details>
      </div>
    </section>
