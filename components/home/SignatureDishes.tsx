...
const controlClass = "inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/15 text-white transition hover:border-brand-400 hover:bg-brand-500/10 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand-400";
...
export default function SignatureDishes() {
  ...
  return (
    <section id="featured-dishes" aria-labelledby="featured-heading" className="bg-[#0c0a09] py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <div className="mb-10 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-brand-400">{t.dishes.eyebrow}</p>
            <h2 id="featured-heading" className="mt-4 text-4xl font-semibold leading-tight text-white md:text-5xl">{t.dishes.title}</h2>
          </div>
          <Link href="/menu" className="inline-flex items-center gap-3 self-start rounded-full border border-brand-500/50 px-6 py-3 text-sm text-white transition hover:bg-ember-700 md:self-auto">
        ...
        <div
          ...
          className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#15120f]"
        >
          <div className="grid md:grid-cols-2" aria-live={autoPlaying ? "off" : "polite"} aria-atomic="true">
            <div className="relative aspect-[4/3] overflow-hidden bg-gradient-to-br from-brand-950/50 via-[#20180f] to-[#100d0b] md:aspect-auto md:min-h-[380px]">
              {dish.image ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-6">
                  <div aria-hidden="true" className="flex h-40 w-40 items-center justify-center rounded-full border border-brand-400/15 bg-brand-400/[0.03] ring-1 ring-brand-400/10 ring-offset-[18px] ring-offset-transparent sm:h-48 sm:w-48">
                    <Utensils size={40} strokeWidth={1} className="text-brand-300/50" />
                  </div>
            ...
            <div className="flex min-h-[330px] flex-col justify-center p-7 sm:p-10 lg:p-12">
              <motion.div key={dish.id} initial={reducedMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
                <p className="text-xs uppercase tracking-[0.25em] text-brand-400">{copy.slide} {String(active + 1).padStart(2, "0")} / {String(featuredDishes.length).padStart(2, "0")}</p>
                ...
                <p className="mt-6 text-xl font-medium text-brand-300">{formatMenuPrice(dish.price, language)}</p>
              </motion.div>
            </div>
          </div>
          <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/10 px-6 py-4 sm:px-10">
            <div className="flex items-center gap-1">
              {featuredDishes.map((item, index) => (
                <button key={item.id} type="button" aria-label={`${copy.slide} ${index + 1}: ${item.name[language]}`} aria-current={index === active ? "true" : undefined} onClick={() => selectSlide(index)} className="flex h-11 w-9 items-center justify-center rounded-full focus-visible:outline-2 focus-visible:outline-brand-400">
                  <span className={`h-1 rounded-full transition-all motion-reduce:transition-none ${index === active ? "w-7 bg-brand-400" : "w-3 bg-zinc-600"}`} />
                </button>
              ))}
            </div>
