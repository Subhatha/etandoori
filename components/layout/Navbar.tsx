...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#0c0a09]/90 backdrop-blur-xl">
      <div className="relative mx-auto flex h-20 w-full max-w-6xl items-center justify-between px-5 sm:px-8 xl:grid xl:grid-cols-[auto_minmax(0,1fr)_auto]">
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-white transition hover:border-brand-500 xl:hidden"
        >
        ...
        {isMenuOpen && (
          <div className="absolute inset-x-0 top-full border-b border-white/10 bg-[#0c0a09]/98 px-5 py-7 shadow-2xl shadow-black/30 sm:px-8 xl:hidden">
            <nav className="flex flex-col items-start gap-5" aria-label={t.nav.mobileNavigation}>
              ...
              <Link href="/order-online" onClick={() => setIsMenuOpen(false)} className="mt-2 rounded-full border border-brand-600 px-6 py-3 text-sm uppercase tracking-[0.2em] text-white transition hover:bg-ember-700">
                {t.nav.orderOnline}
              </Link>
            </nav>
          </div>
